...
Vectors are ways to organize data in R. 

2. What is a data matrix in R? 
A matric is a collection of data, sprted into rows and columns. 
...
```r
temperature_matrix <- matrix(temperature, nrow = 8, byrow = T)
temperature_matrix
```
...
```r
all_temperature_matrix <- cbind(temperature_matrix,average)
...
```r
jills_findings <- all_temperature_matrix[ ,1]
mean(jills_findings)
```
...
```r
susans_findings <- all_temperature_matrix[,2]
mean(susans_findings)
```
